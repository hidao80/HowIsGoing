...
logOutput('api/v1/save: ' . var_export($json, true));

$contents = json_decode($json, true);
logOutput('api/v1/save: ' . var_export($contents, true));

// If there is no user_id in the JSON sent, make an error
// with status code 400 and abort the process.
$user_id = $contents['user_id'];
logOutput('api/v1/save: user_id = ' . $user_id);
if (empty($user_id)) {
...
logOutput('api/v1/save: todo_list = ' . var_export($todo_list, true));
if (empty($todo_list)) {
    logOutput('api/v1/save: todo list is empty');
    http_response_code(400);
    return;
}
...
try {
    foreach ($todo_list as $value) {
        ...
        logOutput('api/v1/save: ' . var_export([$sql, $params], true));
    ...
    $DB->exec('ROLLBACK');
    logOutput('api/v1/save: ' . $ex->getMessage());
